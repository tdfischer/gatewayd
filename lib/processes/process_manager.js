...
function ProcessManager() {
  ...
  if (config.get('HTTP_SERVER')) {
    this.processSet.add(__dirname + '/../../processes/server.js');
  }
  if (config.get('WITHDRAWALS_CALLBACK_URL')) {
    this.processSet.add(__dirname + '/../../processes/withdrawal_callbacks_url.js');
  }
  if (config.get('RIPPLE_REST_PATH')) {
    this.processSet.add(__dirname + '/../../processes/ripple_rest.js');
  }
}
