...
function enqueueOutgoingPayment(options, callback) {
  ...
  async.waterfall([
    function(next) {
      _findOrCreateSenderAddress(next);
    },
    function(senderAddress, next){
      _findOrCreateDestinationAddress(options, function(error, destinationAddress) {
        next(error, destinationAddress, senderAddress);
      });
    },
    function(toAddress, fromAddress, next){
      _recordTransaction(options, toAddress, fromAddress, next);
    }
  ], callback);
}
